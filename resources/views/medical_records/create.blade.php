...
<div class="container-fluid mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="mb-0 fw-bold text-dark">
            <i class="fas fa-file-medical-alt me-2 text-primary"></i> Thêm hồ sơ bệnh án mới
        </h3>
    ...
    <div class="card shadow-lg border-0">
        <div class="card-header bg-primary bg-gradient text-white py-3">
            <h5 class="card-title mb-0"><i class="fas fa-pen-fancy me-2"></i>Thông tin hồ sơ</h5>
        </div>
        
        <div class="card-body p-4">
            <form action="{{ route('medical_records.store') }}" method="POST" id="medicalRecordForm">
                @csrf
                ...
                <div class="row g-3">
                    ...
                    {{-- Bác sĩ & Khoa --}}
                    <div class="col-md-6">
                        <label for="doctor_id" class="form-label fw-bold">Bác sĩ</label>
                    ...
                    <div class="col-md-6">
                        ...
                        <textarea name="symptoms" id="symptoms" rows="3" 
                                  ...
                                  placeholder="Mô tả triệu chứng bệnh nhân...">{{ old('symptoms') }}</textarea>
                        @error('symptoms')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <textarea name="vital_signs" id="vital_signs" rows="3" 
                                  ...
                                  placeholder='Ví dụ: {"temp":"37","bp":"120/80","hr":"72"}'>{{ old('vital_signs') }}</textarea>
                        @error('vital_signs')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="treatment" id="treatment" rows="3" 
                                  ...
                                  placeholder="Mô tả phương pháp điều trị...">{{ old('treatment') }}</textarea>
                        @error('treatment')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                            <option value="chờ_khám" {{ old('status') == 'chờ_khám' ? 'selected' : '' }}>Chờ khám</option>
                            <option value="đang_khám" {{ old('status') == 'đang_khám' ? 'selected' : '' }}>Đang khám</option>
                            <option value="đã_khám" {{ old('status') == 'đã_khám' ? 'selected' : '' }}>Đã khám</option>
                            <option value="hủy" {{ old('status') == 'hủy' ? 'selected' : '' }}>Hủy</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
